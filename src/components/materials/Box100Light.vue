...
const OPERATORS = {
  ADDITION: {
    ...
    ICON: 'mdi-plus',
    TEXT: 'たし算',
    ROW_NUMBERS: [...Array(5).keys()].map(x => x+1),
    COL_NUMBERS: [...Array(5).keys()].map(x => x+1),
    CALC: function(r,c) {
      return r + c;
    }
  },
  SUBTRACT: {
    ...
    TEXT: 'ひき算',
    ROW_NUMBERS: [...Array(5).keys()].map(x => x+1),
    COL_NUMBERS: [...Array(5).keys()].map(x => x+5),
    CALC: function(r,c) {
      return c - r;
    }
  },
...
const COOKIE_KEYS = {
  OPERATOR: 'materials.Box100Light.operator',
};
export default {
  components: {
    BasicInput
  ...
  data(){
    return{
        ...
        selOperatorOptions: [OPERATORS.ADDITION, OPERATORS.SUBTRACT],
        printed: false,
